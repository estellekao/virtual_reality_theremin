...
namespace Leap.Unity
{
     ...
     * @since 4.1.2
     */
    public class GetLeapPalmVolume : Detector
    {
        /**
        ...
        {
            Hand hand;
            // Play object already called by the right hand.
            while (true)
            {
                ...
                if (HandModel != null && HandModel.IsTracked)
                {
                    ...
                    if (hand != null)
                    {
                        ...
                        for (int f = 0; f < 5; f++)
                        {
                            ...
                            if (FingerDistToBase < MinFingerToBase) {
                                MinFingerToBase = FingerDistToBase;
                                //print(MinFingerToBase);
                            }
                        }
        ...
        private float getDistToBase(Finger finger)
        {
            ...
            return 1.2f - finger.TipPosition[2];
        }

        private void playvolume(float MinFingerToBase)
        {
            float pitch = (.4f/ MinFingerToBase);
            print(pitch);
            FindObjectOfType<AudioManager>().ChangePitch("A", pitch);
            //FindObjectOfType<AudioManager>().ChangePitch("A", MinFingerToPoleDist * 10f);
            /*
            if (MinFingerToPoleDist < 0.1f)
            {
